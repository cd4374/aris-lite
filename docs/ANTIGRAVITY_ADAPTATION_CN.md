...
- 关键文件: 04_NARRATIVE_REPORT.md, 01_IDEA_REPORT.md
```
...
如果 ARIS 技能已安装（§3.1），Antigravity 会自动发现并激活 `aris-3-1-auto-review-loop` 技能。
...
```
读取 skills/aris-3-1-auto-review-loop/SKILL.md 并按照其指令执行。
...
**Claude Code：**
```
/aris-0-2-idea-discovery "your research direction"
```
...
按顺序执行以下子技能：
1. 读取并执行 skills/aris-1-1-research-lit/SKILL.md —— 文献综述
2. 读取并执行 skills/aris-1-4-idea-creator/SKILL.md —— 头脑风暴
3. 读取并执行 skills/aris-1-5-novelty-check/SKILL.md —— 新颖性验证
4. 读取并执行 skills/aris-1-6-research-review/SKILL.md —— 深度评审
5. 读取并执行 skills/aris-1-9-research-refine-pipeline/SKILL.md —— 方法精化 + 实验规划
```
...
**Antigravity 等价：**
```
读取并执行 skills/aris-0-3-experiment-bridge/SKILL.md。
读取 02_EXPERIMENT_PLAN.md（回退：`refine-logs/EXPERIMENT_PLAN.md`）并实现实验。
通过 skills/aris-2-1-run-experiment/SKILL.md 部署到 GPU。
```
...
**Claude Code：**
```
/aris-3-1-auto-review-loop "your paper topic"
```

**Antigravity 等价：**
```
读取并执行 skills/aris-3-1-auto-review-loop/SKILL.md。
对 "your paper topic" 运行自动评审循环。
读取项目叙事文档、记忆文件和实验结果。
使用 MCP 工具 mcp__codex__codex 进行外部审阅。
```

> **注意：** 如果使用 `llm-chat` MCP，把 `mcp__codex__codex` 替换为 `mcp__llm-chat__chat`。或使用适配版技能：`skills/aris-3-4-auto-review-loop-llm/SKILL.md`。
...
**Antigravity 等价：**
```
读取并执行 skills/aris-4-7-paper-writing/SKILL.md。
输入：项目根目录的 04_NARRATIVE_REPORT.md（回退：`NARRATIVE_REPORT.md`）。

按顺序执行以下子技能：
1. 读取并执行 skills/aris-4-1-paper-plan/SKILL.md —— 大纲 + claims-evidence matrix
2. 读取并执行 skills/aris-4-2-paper-figure/SKILL.md —— 生成图表
3. 读取并执行 skills/aris-4-4-paper-write/SKILL.md —— 写 LaTeX 章节
4. 读取并执行 skills/aris-4-5-paper-compile/SKILL.md —— 编译 PDF
5. 读取并执行 skills/aris-4-6-auto-paper-improvement-loop/SKILL.md —— 审阅与润色
```
...
| 阶段 | 执行方式 | 产出文件 |
|------|---------|---------|
| 1 | 创意发现：`skills/aris-0-2-idea-discovery/SKILL.md` + 研究方向 | `01_IDEA_REPORT.md`, `01_FINAL_PROPOSAL.md`, `02_EXPERIMENT_PLAN.md` |
| 2 | 实验桥接：`skills/aris-0-3-experiment-bridge/SKILL.md` | `02_EXPERIMENT_RESULTS.md`、更新后的 `02_EXPERIMENT_TRACKER.md` |
| 3 | 自动评审：`skills/aris-3-1-auto-review-loop/SKILL.md` | `03_AUTO_REVIEW.md`, `04_NARRATIVE_REPORT.md` |
| 4 | 论文写作：`skills/aris-4-7-paper-writing/SKILL.md` + `04_NARRATIVE_REPORT.md` | `paper/` 目录，`06_SUBMISSION_GATE.md` |
...
| `03_AUTO_REVIEW.md` | 累计评审日志 | auto-review-loop |
| `01_IDEA_REPORT.md` | 创意筛选与排名 | idea-discovery |
| `05_PAPER_PLAN.md` | 论文大纲 + claim-evidence matrix | paper-plan |
| `01_FINAL_PROPOSAL.md` | 精化后的方法提案 | research-refine |
| `02_EXPERIMENT_PLAN.md` | 实验路线图 | experiment-plan |

中断恢复示例：

```
读取 skills/aris-3-1-auto-review-loop/SKILL.md，然后读取 REVIEW_STATE.json 和 03_AUTO_REVIEW.md。
从保存的状态恢复自动评审循环。
```
...
- 预览 `/aris-4-2-paper-figure` 生成的图表
- `/aris-1-1-research-lit` 中的 arXiv 搜索
- 查看 `/aris-4-5-paper-compile` 编译的 PDF
...
- `/aris-3-1-auto-review-loop` 的评审发现在未来会话中可用
- 实验配置和结果持久保存在知识条目中
...
```
# 文献综述
读取 skills/aris-1-1-research-lit/SKILL.md，搜索 "discrete diffusion models" 相关论文。

# 创意发现（完整流程）
读取 skills/aris-0-2-idea-discovery/SKILL.md，对 "factorized gap in discrete diffusion LMs" 运行创意发现。

# 单次深度评审
读取 skills/aris-1-6-research-review/SKILL.md，评审我的研究：[描述或指向文件]。

# 自动评审循环
读取 skills/aris-3-1-auto-review-loop/SKILL.md，运行自动评审循环。课题："your paper topic"。

# 论文写作
读取 skills/aris-4-7-paper-writing/SKILL.md，根据 04_NARRATIVE_REPORT.md 写论文。

# 部署实验
读取 skills/aris-2-1-run-experiment/SKILL.md 和 GEMINI.md。
...
- [ ] 快速测试：`读取 skills/aris-1-6-research-review/SKILL.md 并评审我的项目`

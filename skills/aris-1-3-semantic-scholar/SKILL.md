---
name: aris-1-3-semantic-scholar
description: Search published venue papers (IEEE, ACM, Springer, etc.) via Semantic Scholar API. Complements /aris-1-2-arxiv (preprints) with citation counts, venue metadata, and TLDR. Use when user says "search semantic scholar", "find IEEE papers", "find journal papers", "venue papers", "citation search", or wants published literature beyond arXiv preprints.
...
This skill is the **published venue** counterpart to `/aris-1-2-arxiv`:

| Skill | Source | Best for |
|-------|--------|----------|
| `/aris-1-2-arxiv` | arXiv API | Latest preprints, cutting-edge unrefereed work |
| `/aris-1-3-semantic-scholar` | Semantic Scholar API | **Published** journal/conference papers (IEEE, ACM, Springer, etc.) with citation counts, venue info, TLDR |
...
> - `/aris-1-3-semantic-scholar "topic" - max: 20` — return up to 20 results
> - `/aris-1-3-semantic-scholar "topic" - type: journal` — only journal articles
> - `/aris-1-3-semantic-scholar "topic" - type: conference` — only conference papers
> - `/aris-1-3-semantic-scholar "topic" - min-citations: 50` — only highly-cited papers
> - `/aris-1-3-semantic-scholar "topic" - year: 2022-` — papers from 2022 onward
> - `/aris-1-3-semantic-scholar "topic" - fields: all` — remove default field-of-study filter
> - `/aris-1-3-semantic-scholar "topic" - sort: citations` — bulk search sorted by citation count
> - `/aris-1-3-semantic-scholar "DOI:10.1109/..."` — fetch a single paper by DOI
...
[ -z "$SCRIPT" ] && SCRIPT=$(find ~/.claude/skills/aris-1-3-semantic-scholar/ -name "semantic_scholar_fetch.py" 2>/dev/null | head -1)
```
...
- If present → paper is also on arXiv. Note this in output but do NOT re-fetch via `/aris-1-2-arxiv`.
...
```text
/aris-1-2-arxiv "topic"           - search arXiv preprints (complements this search)
/aris-1-1-research-lit "topic"    - multi-source review: Zotero + local PDFs + arXiv + S2
/aris-1-5-novelty-check "idea"    - verify novelty against literature
```
...
- If the S2 API is unreachable, suggest using `/aris-1-2-arxiv` or `/aris-1-1-research-lit "topic" - sources: web` as fallback.

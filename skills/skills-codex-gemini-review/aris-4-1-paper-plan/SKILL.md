...
1. **`04_NARRATIVE_REPORT.md`** (preferred) or `NARRATIVE_REPORT.md` / `STORY.md` — research narrative with claims and evidence
2. **`03_AUTO_REVIEW.md`** (preferred) or `GPT54_AUTO_REVIEW.md` / `AUTO_REVIEW.md` — auto-review loop conclusions
3. **Experiment results** — JSON files in `figures/`, screen logs, tables
4. **`01_IDEA_REPORT.md`** (preferred) or `IDEA_REPORT.md` — from idea-discovery pipeline (if applicable)
...
**First check for `03_CLAIMS_FROM_RESULTS.md`** — if it exists (generated by `/aris-3-2-result-to-claim` at the end of Workflow 2), use it as the starting point for claims. This file contains validated claims already mapped to experiment evidence. Merge with any additional claims from the narrative documents below. Fall back to `CLAIMS_FROM_RESULTS.md` only when the canonical file is absent.

If `03_CLAIMS_FROM_RESULTS.md` does not exist, extract claims from scratch:
...
Save the final outline to `05_PAPER_PLAN.md` in the project root (fallback readers may still consume `PAPER_PLAN.md` when canonical files are absent):

...
ARIS experiments run remotely in screen/tmux sessions. Current monitoring (`/aris-2-2-monitor-experiment`) is **on-demand** — you have to actively ask "how are my experiments doing?". Between checks:
...
Or if using rsync for code sync (as in `/aris-2-1-run-experiment`), just include `tools/` in the sync.
...
### With `/aris-2-1-run-experiment`

After deploying an experiment with `/aris-2-1-run-experiment`, register it with watchdog:

```bash
# /aris-2-1-run-experiment launches screen session "exp01"
# Then register it:
python3 tools/watchdog.py --register '{"name":"exp01","type":"training","session":"exp01","gpus":[0,1]}'
```

### With `/aris-2-2-monitor-experiment`

`/aris-2-2-monitor-experiment` does deep inspection (log parsing, W&B metrics, result collection). Watchdog does continuous surface-level health checks. They complement each other:

- **Watchdog** — "is it alive and using GPU?" (runs 24/7, low overhead)
- **`/aris-2-2-monitor-experiment`** — "what are the actual results?" (on-demand, detailed)

...
ARIS 已经将部分状态持久化到文件（`REVIEW_STATE.json`、`03_AUTO_REVIEW.md`；回退：`AUTO_REVIEW.md`），但**没有系统性机制确保 LLM 在恢复时去读这些文件**。压缩后，它经常忘记。
...
工作流 1（`/aris-0-2-idea-discovery`）完成后，`01_IDEA_REPORT.md` 是 canonical 的 idea 排名产物（回退：`IDEA_REPORT.md`）。一旦选定一个进入实现阶段，把所有候选都留在上下文中会浪费 LLM 的工作记忆、降低输出质量。

**`docs/research_contract.md`** 解决这个问题：只提取*当前正在做的那一个 idea* 到一份聚焦的工作文档——claim、实验设计、baseline、结果。新 session 读这个，而不是读整个 `01_IDEA_REPORT.md`（回退：`IDEA_REPORT.md`）。模板见 [`templates/RESEARCH_CONTRACT_TEMPLATE.md`](../templates/RESEARCH_CONTRACT_TEMPLATE.md)。
...
2. `docs/research_contract.md`（当前 idea 的聚焦上下文 — 不是整个 `01_IDEA_REPORT.md`）
...
| `progress-remind.sh` | `PostToolUse`（Write/Edit） | 代码修改后提醒更新 `02_EXPERIMENT_TRACKER.md` |
...
case "$STAGE" in
  idea-discovery)
    OUTPUT="$OUTPUT\n\n[session-restore] Stage: idea-discovery. Resume with /aris-0-2-idea-discovery or /aris-1-1-research-lit." ;;
  implementation)
    OUTPUT="$OUTPUT\n\n[session-restore] Stage: implementation. Resume with /aris-0-3-experiment-bridge." ;;
  training)
    OUTPUT="$OUTPUT\n\n[session-restore] Stage: training. Check experiments, then /aris-3-1-auto-review-loop if results are ready." ;;
  paper)
    OUTPUT="$OUTPUT\n\n[session-restore] Stage: paper. Resume with /aris-4-7-paper-writing or /aris-4-4-paper-write." ;;
esac
...
echo "[progress-remind] 已累计 ${COUNT} 次代码修改。如有阶段性进展，请更新 02_EXPERIMENT_TRACKER.md 和 Pipeline Status。"
HOOKEOF

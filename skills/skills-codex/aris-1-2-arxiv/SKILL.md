...
- **FETCH_SCRIPT** - `tools/aris-1-2-arxiv_fetch.py` relative to the ARIS install, or the same path relative to the current project. Fall back to inline Python if not found.

> Overrides (append to arguments):
> - `/aris-1-2-arxiv "attention mechanism" - max: 20` - return up to 20 results
> - `/aris-1-2-arxiv "2301.07041" - download` - download a specific paper by ID
> - `/aris-1-2-arxiv "query" - dir: literature/` - save PDFs to a custom directory
> - `/aris-1-2-arxiv "query" - download: all` - download all result PDFs
...
```text
/aris-1-1-research-lit "topic"     - multi-source review: Zotero + Obsidian + local PDFs + web
/aris-1-5-novelty-check "idea"     - verify your idea is novel against these papers
```

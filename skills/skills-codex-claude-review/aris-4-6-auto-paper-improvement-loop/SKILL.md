...
description: "Autonomously improve a generated paper via Claude review through claude-review MCP → implement fixes → recompile, for up to 3 rounds with a strict quality gate. Use when user says \"改论文\", \"improve paper\", \"论文润色循环\", \"auto improve\", or wants to iteratively polish a generated paper."
---
...
This skill is designed to run **after** Workflow 3 (`/aris-4-1-paper-plan` → `/aris-4-2-paper-figure` → `/aris-4-4-paper-write` → `/aris-4-5-paper-compile`). It takes a compiled paper and iteratively improves it through external LLM review.

Unlike `/aris-3-1-auto-review-loop` (which iterates on **research** — running experiments, collecting data, rewriting narrative), this skill iterates on **paper writing quality** — fixing theoretical inconsistencies, softening overclaims, adding missing content, and improving presentation.
...
Parse user response same as `/aris-3-1-auto-review-loop`: approve / custom instructions / skip / stop.
...
Use `mcp__claude-review__review_start-reply` with the saved `review_thread_id`:
...
- **Use `mcp__claude-review__review_start-reply`** for Round 2 to maintain conversation context

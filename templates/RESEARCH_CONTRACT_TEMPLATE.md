...
> **A focused working document for the currently selected idea.** Created when an idea is chosen from `01_IDEA_REPORT.md` (fallback: `IDEA_REPORT.md`), updated throughout implementation and training. This is what the LLM reads on session recovery — not the full idea report with all 8-12 candidates.
>
> **Why this file exists:** After brainstorming, `01_IDEA_REPORT.md` contains the canonical candidate idea set (fallback: `IDEA_REPORT.md`). Keeping all of them in context pollutes the LLM's working memory and degrades output quality. This contract extracts *only the active idea* into a standalone document, so new sessions and post-compaction recovery load focused context instead of the entire idea pool.

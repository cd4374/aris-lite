...
Given a broad research direction from the user, systematically generate, validate, and rank concrete research ideas. This skill composes with `/aris-1-1-research-lit`, `/aris-1-5-novelty-check`, and `/aris-1-6-research-review` to form a complete idea discovery pipeline.
...
2. **Novelty quick-check**: For each idea, do 2-3 targeted searches to see if it's already been done. Full `/aris-1-5-novelty-check` comes later for survivors.
...
2. **Deploy in parallel**: Use `/aris-2-1-run-experiment` to launch pilots on different GPUs simultaneously:
   ```
...
3. **Collect results**: Use `/aris-2-2-monitor-experiment` to check progress. If any pilot exceeds PILOT_TIMEOUT_HOURS, kill it and collect partial results. Once all pilots complete (or timeout), compare:
...
- [ ] If confirmed, invoke /aris-3-1-auto-review-loop for full iteration
```
...
```
/aris-1-4-idea-creator "direction"     → ranked ideas
/aris-1-5-novelty-check "top idea"     → deep novelty verification (already done in Phase 4, but user can re-run)
/aris-1-6-research-review "top idea"   → external critical feedback

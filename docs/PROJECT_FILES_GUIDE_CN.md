...
├── 01_IDEA_REPORT.md                  # canonical 创意报告
├── 01_FINAL_PROPOSAL.md               # canonical 方法提案
├── 02_EXPERIMENT_PLAN.md              # canonical 实验设计（claim + blocks）
├── 02_EXPERIMENT_TRACKER.md           # canonical 执行清单（TODO → DONE）
├── 03_AUTO_REVIEW.md                  # canonical 审稿循环日志
├── REVIEW_STATE.json                  # 审稿循环恢复状态
└── 04_NARRATIVE_REPORT.md             # canonical 叙事 handoff
```
...
| 文件 | 创建者 | 定位 |
|------|--------|------|
| `01_IDEA_REPORT.md` | `/aris-0-2-idea-discovery` | canonical brainstorm / ranking 产出：idea 排名、pilot 结果、淘汰原因 |
| `01_FINAL_PROPOSAL.md` | `/aris-1-7-research-refine` | canonical 精化后的方法提案 |
| `02_EXPERIMENT_PLAN.md` | `/aris-1-8-experiment-plan` | canonical 实验设计：claim 映射、实验块、执行顺序、算力预算 |
| `02_EXPERIMENT_TRACKER.md` | `/aris-1-8-experiment-plan` | canonical 执行清单：run ID、状态（TODO→DONE）、一句话 notes |
| `03_AUTO_REVIEW.md` | `/aris-3-1-auto-review-loop` | canonical 审稿循环累积日志：评分、reviewer 原始响应、采取的行动 |
| `04_NARRATIVE_REPORT.md` | `/aris-3-1-auto-review-loop` | canonical 论文叙事 handoff |
| `REVIEW_STATE.json` | `/aris-3-1-auto-review-loop` | 上下文压缩恢复状态 |
...
```
01_IDEA_REPORT.md                 （canonical idea 排名与 pilot 结果）
...
```
02_EXPERIMENT_PLAN.md             （跑什么 — 设计）
  ↓
02_EXPERIMENT_TRACKER.md          （执行状态 — TODO/RUNNING/DONE）
  ↓ 实验完成
...
| 文件 | 什么时候写 | 更新频率 |
|------|----------|----------|
| `IDEA_CANDIDATES.md` | `/aris-0-2-idea-discovery` 完成后创建；idea kill/selection 时更新 | 每次 idea 切换 |
...
**不要**在非切换 idea 时读 `01_IDEA_REPORT.md` 或 `IDEA_CANDIDATES.md`。

## 分工原则

| 问题 | 答案 |
|------|------|
| brainstorm 的 idea 放哪？ | `01_IDEA_REPORT.md`（canonical）→ `IDEA_CANDIDATES.md`（筛选后） |
| 当前 idea 的完整上下文放哪？ | `docs/research_contract.md` |
| "实验 X 正在跑"放哪？ | `02_EXPERIMENT_TRACKER.md` |
| "实验 X 准确率 95.2"放哪？ | `EXPERIMENT_LOG.md` |
| "lr=1e-4 在数据集 X 上发散"放哪？ | `findings.md` |
| "reviewer 说加 ablation"放哪？ | `03_AUTO_REVIEW.md` |

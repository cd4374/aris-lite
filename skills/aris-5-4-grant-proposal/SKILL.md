...
```
/aris-1-1-research-lit → /aris-1-5-novelty-check → [structure design] → [draft] → /aris-1-6-research-review → [revise] → GRANT_PROPOSAL.md
  (survey)      (verify gap)     (aims + matrix)     (prose)    (panel review)     (fix)      (done!)
```

**This is a parallel branch, not part of the linear Workflow 1→1.5→2→3 pipeline.** After `/aris-0-2-idea-discovery` produces validated ideas, the user can either:
- Go to `/aris-0-3-experiment-bridge` → `/aris-3-1-auto-review-loop` → `/aris-4-7-paper-writing` (implement & publish)
- Go to `/aris-5-4-grant-proposal` (write funding application first, then implement after funding)

```
                    ┌→ /aris-0-3-experiment-bridge → /aris-3-1-auto-review-loop → /aris-4-7-paper-writing  (publish track)
/aris-0-2-idea-discovery ────┤
                    └→ /aris-5-4-grant-proposal → [get funded] → /aris-0-3-experiment-bridge → ...  (funding track)
```
...
> 💡 These are defaults. Override by telling the skill, e.g., `/aris-5-4-grant-proposal "topic — NSF CAREER, latex output"` or `/aris-5-4-grant-proposal "topic — NSFC Youth, language: English"`.
...
```
/aris-5-4-grant-proposal "topic — GENERIC, sections: Background|Methods|Impact, language: English"
```
...
Invoke `/aris-1-1-research-lit` to ground the proposal in real literature, then search for competing funded projects:

```
/aris-1-1-research-lit "$ARGUMENTS"
```

**What this does:**
- Reuse existing surveys if `/aris-1-1-research-lit` was already run and notes exist
- Otherwise invoke `/aris-1-1-research-lit` for multi-source literature search (arXiv, Scholar, Zotero, local PDFs)
...
- Run `/aris-1-5-novelty-check` on the proposed research direction to verify the gap is real:
  ```
  /aris-1-5-novelty-check "[proposed gap statement]"
  ```
...
Invoke `/aris-1-6-research-review` to get critical feedback on the proposal structure before drafting:

```
/aris-1-6-research-review "[GRANT_TYPE] [GRANT_SUBTYPE] proposal structure:
Gap: [gap statement]
...
For AI-generated publication-quality figures, invoke `/aris-4-3-paper-illustration`:

```
/aris-4-3-paper-illustration "Overview diagram showing [aims relationship + shared resources] for grant proposal"
```
...
Invoke `/aris-1-6-research-review` on the complete draft for grant-type-specific evaluation:

```
/aris-1-6-research-review "Complete [GRANT_TYPE] [GRANT_SUBTYPE] proposal draft. Evaluate as a [GRANT_TYPE] review panelist using official criteria. [PASTE FULL PROPOSAL TEXT]"
```
...
```
/aris-5-4-grant-proposal "topic — KAKENHI Start-up, sources: zotero, arxiv download: true"
```
...
|-----------|:-----:|---------|
| `/aris-1-1-research-lit` | 1 | Literature survey (if not already done) |
| `/aris-1-5-novelty-check` | 1 | Verify the gap is real |
| `/aris-1-6-research-review` | 2, 4 | Structural review + full draft review |
| `/aris-4-3-paper-illustration` | 3 | Generate proposal figures (optional) |

### Funding Track (this skill's primary use case)

```
/aris-0-2-idea-discovery "direction"              ← Workflow 1: find validated ideas
/aris-1-7-research-refine "idea"                  ← sharpen the method
/aris-5-4-grant-proposal "idea — KAKENHI"         ← this skill: write the grant proposal
                                          ← [submit & get funded]
/aris-0-3-experiment-bridge                       ← implement experiments with funding
/aris-3-1-auto-review-loop "results"              ← Workflow 2: iterate until submission-ready
/aris-4-7-paper-writing                           ← Workflow 3: write the paper
```

### Publish Track (skip this skill)

```
/aris-0-2-idea-discovery → /aris-0-3-experiment-bridge → /aris-3-1-auto-review-loop → /aris-4-7-paper-writing → submit
```

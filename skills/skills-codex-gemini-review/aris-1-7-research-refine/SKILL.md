...
- Default to **1-3 core experiment blocks** and leave the full execution roadmap to `/aris-1-8-experiment-plan`.
...
## Next Steps
- If READY: proceed to `/aris-1-8-experiment-plan` for a full experiment roadmap, then `/aris-2-1-run-experiment`
- If REVISE: manually address the remaining mechanism weaknesses, then re-run `/aris-1-7-research-refine`
- If RETHINK: revisit the core mechanism, possibly with `/aris-1-4-idea-creator`
```
...
- **Save `threadId` from Phase 2** and use `mcp__gemini-review__review_start-reply` for later rounds.
...
```
/aris-1-9-research-refine-pipeline              -> one-shot refine + experiment planning
/aris-1-4-idea-creator "direction"       -> candidate ideas
/aris-1-7-research-refine "PROBLEM: ... | APPROACH: ..."  <- you are here
/aris-1-8-experiment-plan                -> detailed experiment roadmap
/aris-2-1-run-experiment                 -> execute the chosen method
/aris-3-1-auto-review-loop               -> iterate on results and paper
```

Typical flow:

1. `/aris-1-4-idea-creator` or local reading gives you a problem and a vague method direction
2. `/aris-1-7-research-refine` turns that into an anchored, elegant, frontier-aware method plan
3. `/aris-1-8-experiment-plan` turns the final proposal into a detailed claim-driven experiment roadmap
4. `/aris-1-9-research-refine-pipeline` is the one-shot wrapper when the user wants both stages in a single request
5. `/aris-2-1-run-experiment` executes the chosen runs

...
```
/aris-1-1-research-lit → /aris-1-4-idea-creator → /aris-1-5-novelty-check → /aris-1-6-research-review → /aris-1-9-research-refine-pipeline
...
> 💡 These are defaults. Override by telling the skill, e.g., `/aris-0-2-idea-discovery "topic" — pilot budget: 4h per idea, 20h total` or `/aris-0-2-idea-discovery "topic" — arxiv download: true`.

## Pipeline

### Phase 1: Literature Survey

Invoke `/aris-1-1-research-lit` to map the research landscape:
...
- **User requests changes** (e.g., "focus more on X", "ignore Y", "too broad") → refine the search with updated queries, re-run `/aris-1-1-research-lit` with adjusted scope, and present again. Repeat until the user is satisfied.

### Phase 2: Idea Generation + Filtering + Pilots

Invoke `/aris-1-4-idea-creator` with the landscape context:
...
**What this does:**
- Brainstorm 8-12 concrete ideas via the Gemini-backed `/aris-1-4-idea-creator` overlay
...
```
/aris-1-5-novelty-check "[top idea 1 description]"
/aris-1-5-novelty-check "[top idea 2 description]"
```

**What this does:**
- Multi-source literature search (arXiv, Scholar, Semantic Scholar)
- Cross-verify with the Gemini-backed `/aris-1-5-novelty-check` overlay
...
```
/aris-1-6-research-review "[top idea with hypothesis + pilot results]"
```
...
```
/aris-1-9-research-refine-pipeline "[top idea description + pilot results + reviewer feedback]"
```
...
- **User requests changes** → pass feedback to `/aris-1-7-research-refine` for another round.
- **Lite mode:** If reviewer score < 6 or pilot was weak, run `/aris-1-7-research-refine` only (skip `/aris-1-8-experiment-plan`) and note remaining risks in the report.
...
- Reviewer score: X/10
- Next step: implement full experiment → /aris-3-1-auto-review-loop
...
- [ ] /aris-3-1-auto-review-loop to iterate until submission-ready
- [ ] Or invoke /aris-0-1-research-pipeline for the complete end-to-end flow
```
...
```
/aris-0-2-idea-discovery "direction"         ← you are here (Workflow 1, includes method refinement + experiment planning)
/aris-2-1-run-experiment                     ← deploy experiments from the plan
/aris-3-1-auto-review-loop "top idea"        ← Workflow 2: iterate until submission-ready

Or use /aris-0-1-research-pipeline for the full end-to-end flow.
```

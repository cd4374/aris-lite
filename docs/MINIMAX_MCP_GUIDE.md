...
|------|-----------|--------|-------------|
| 原版 | `aris-3-1-auto-review-loop` | "auto review loop" | Codex MCP (OpenAI) |
| MiniMax版 | `aris-3-5-auto-review-loop-minimax` | "auto review loop minimax" | MiniMax API |
...
`aris-3-5-auto-review-loop-minimax` 只是**一个** skill 的 MiniMax 版。项目中有 **12 个 skill** 使用 Codex MCP (`mcp__codex__codex`) 调用 GPT-5.4 做审查。如果你想全面切换到 MiniMax，需要让 Claude Code 把它们全部改写。

安装完 MCP 服务器后，在 Claude Code 对话中执行：

```
Read skills/aris-3-5-auto-review-loop-minimax/SKILL.md as a reference.
...
2. 参考 `aris-3-5-auto-review-loop-minimax` 的写法（MCP 优先 + curl fallback）
...
```
/aris-3-5-auto-review-loop-minimax
```

...
After the experiment completes (detected via `/aris-2-2-monitor-experiment` or screen session ending):
...
> **Vast.ai setup**: Run `pip install vastai && vastai set api-key YOUR_KEY`. Upload your SSH public key at https://cloud.vast.ai/manage-keys/. Set `gpu: vast` in your `CLAUDE.md` — `/aris-2-1-run-experiment` will automatically rent an instance, run the experiment, and destroy it when done.

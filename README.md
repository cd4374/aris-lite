...
```
/aris-0-1-research-pipeline "离散扩散语言模型的 factorized gap"
```
...
| `auto experiment` | `false` | 自动跑补充实验（`/aris-0-3-experiment-bridge`） |
...
```
/aris-5-2-paper-slides "paper/"     # → Beamer PDF + PPTX + 演讲稿 + Q&A 预案
/aris-5-3-paper-poster "paper/"     # → A0/A1 海报 PDF + 可编辑 PPTX + SVG
```

> *💡 从 idea 到论文到讲台到 rebuttal——一条工具链。🌱*
> 以上是全流程——你也可以单独用任何一个工作流。已有 idea？直接进工作流 1.5。有结果了？跳到工作流 3。见[快速开始](#-快速开始)查看所有命令，[分层工作流导航](#-分层工作流导航)了解完整流程。
...
# 3. 在 Claude Code 中使用
claude
> /aris-0-2-idea-discovery "你的研究方向"              # 工作流 1 — 方向要具体！不要 "NLP"，要 "离散扩散语言模型的 factorized gap"
> /aris-0-3-experiment-bridge                         # 工作流 1.5 — 有计划了？实现 + 部署 + 收结果
> /aris-3-1-auto-review-loop "你的论文主题或范围"         # 工作流 2：审稿 → 修复 → 再审，一夜完成
> /aris-4-7-paper-writing "NARRATIVE_REPORT.md"       # 工作流 3：研究叙事 → 精修 PDF
> /aris-5-1-rebuttal "paper/ + reviews" — venue: ICML  # 工作流 4：解析 review → 起草 rebuttal → follow-up
> /aris-0-1-research-pipeline "你的研究方向"            # 全流程：工作流 1 → 1.5 → 2 → 3 端到端
```
...
> | `wandb` | `false` | 自动给实验脚本加 W&B 日志。设 `true` + 在 CLAUDE.md 配 `wandb_project`。`/aris-2-2-monitor-experiment` 从 W&B 拉训练曲线 |
...
>
> ```
> /aris-0-1-research-pipeline "你的课题" — AUTO_PROCEED: false                          # 在 idea 选择关卡暂停
> /aris-0-1-research-pipeline "你的课题" — human checkpoint: true                       # 每轮 review 后暂停，可给修改意见
> /aris-0-1-research-pipeline "你的课题" — sources: zotero, web                         # 只搜 Zotero + 网络（跳过本地 PDF）
> /aris-0-1-research-pipeline "你的课题" — arxiv download: true                         # 文献调研时下载最相关的 arXiv PDF
> /aris-0-1-research-pipeline "你的课题" — AUTO_PROCEED: false, human checkpoint: true  # 组合使用
> ```
...
- 📊 **31 个可组合 skill** — 自由混搭，或串联为完整流水线（`/aris-0-2-idea-discovery`、`/aris-3-1-auto-review-loop`、`/aris-4-7-paper-writing`、`/aris-0-1-research-pipeline`）
...
- 🧩 **可扩展** — 欢迎贡献领域专用 skill！添加一个 `SKILL.md` 即可提 PR。参见[社区 skills](#-全部-skills)，如 [`aris-8-3-dse-loop`](skills/aris-8-3-dse-loop/SKILL.md)（体系结构/EDA）
...
- **探索新方向（比如写 survey）？** 从工作流 1 开始 → `/aris-0-2-idea-discovery`
- **有计划了，需要实现和跑实验？** 工作流 1.5 → `/aris-0-3-experiment-bridge`
- **已有结果，需要迭代改进？** 工作流 2 → `/aris-3-1-auto-review-loop`
- **准备写论文了？** 工作流 3 → `/aris-4-7-paper-writing`（或分步：`/aris-4-1-paper-plan` → `/aris-4-2-paper-figure` → `/aris-4-4-paper-write` → `/aris-4-5-paper-compile` → `/aris-4-6-auto-paper-improvement-loop`）
- **全流程？** 工作流 1 → 1.5 → 2 → 3 → `/aris-0-1-research-pipeline`，从文献调研一路到投稿
...
```
/aris-1-1-research-lit → /aris-1-4-idea-creator → /aris-1-5-novelty-check → /aris-1-7-research-refine → /aris-0-3-experiment-bridge → /aris-3-1-auto-review-loop → /aris-4-1-paper-plan → /aris-4-2-paper-figure → /aris-4-4-paper-write → /aris-4-6-auto-paper-improvement-loop → 投稿
...
还没有具体 idea？给一个研究方向就行——`/aris-0-2-idea-discovery` 搞定剩下的：
...
**涉及 Skills：** `aris-1-1-research-lit` + `aris-1-4-idea-creator` + `aris-1-5-novelty-check` + `aris-1-6-research-review` + `aris-1-9-research-refine-pipeline`

> 💡 **一键调用：** `/aris-0-2-idea-discovery "你的研究方向"` 自动跑完整个工作流 1。
...
```
1. /aris-1-1-research-lit "discrete diffusion models"    ← Zotero→Obsidian→本地→网络，整理全景
   /aris-1-1-research-lit "topic" — sources: zotero, web  ← 或指定只搜部分源
   /aris-1-1-research-lit "topic" — arxiv download: true   ← 同时下载最相关的 arXiv PDF
2. /aris-1-4-idea-creator "DLLMs post training"     ← 自动生成 8-12 个 idea，筛选排序
3. 选 top 2-3 个 idea
4. /aris-1-5-novelty-check "top idea"                     ← 查新：有没有人做过？
5. /aris-1-6-research-review "top idea"                   ← 让外部 LLM 批判你的想法
6. /aris-1-7-research-refine "top idea"                   ← 冻结问题锚点 + 精炼方法
7. /aris-1-8-experiment-plan                              ← claim-driven 实验路线图
8. /aris-2-1-run-experiment → /aris-3-1-auto-review-loop           ← 闭环！
```
...
已有实验计划（来自工作流 1 或自己写的）？`/aris-0-3-experiment-bridge` 一键搞定：
...
5. 🚀 **部署**完整实验到 GPU（`/aris-2-1-run-experiment`）
...
│         │                                                        │
│         ▼                                                        │
│   准备好进入 /aris-3-1-auto-review-loop                                    │
└─────────────────────────────────────────────────────────────────┘
```

**涉及 Skills：** `aris-0-3-experiment-bridge` + `aris-2-1-run-experiment` + `aris-2-2-monitor-experiment`

> 💡 **一键调用：** `/aris-0-3-experiment-bridge` 自动读取 `refine-logs/EXPERIMENT_PLAN.md`。也可指定：`/aris-0-3-experiment-bridge "my_plan.md"`。

> ⚙️ `CODE_REVIEW`、`AUTO_DEPLOY`、`SANITY_FIRST`、`MAX_PARALLEL_RUNS` 均可配置——见[自定义](#%EF%B8%8F-自定义)。

### 3.x Review 阶段：自动科研循环 🔁
...
**涉及 Skills：** `aris-3-1-auto-review-loop` + `aris-1-6-research-review` + `aris-1-5-novelty-check` + `aris-2-1-run-experiment` + `aris-3-3-analyze-results` + `aris-2-2-monitor-experiment`

> 💡 **一键调用：** `/aris-3-1-auto-review-loop "你的论文主题"` 自动跑完整个工作流 2。
>
> **传什么参数？** 简短的主题或范围就够——skill 会自动读取项目中的叙事文档（`NARRATIVE_REPORT.md`）、memory 文件、实验结果和历史 review，为 GPT-5.4 组装完整上下文。示例：
> - `/aris-3-1-auto-review-loop "离散扩散语言模型的 factorized gap"` — 宽泛主题，skill 自动搜集
> - `/aris-3-1-auto-review-loop "重点看第 3-5 节，CRF 结果偏弱"` — 指定范围 + 提示
> - `/aris-3-1-auto-review-loop` — 也行：skill 读项目文件自动推断主题

```
外部 LLM 评审 → Claude Code 实现修复 → /aris-2-1-run-experiment 部署 → 收结果 → 再评审 → 循环
                ↑ 需要新方向时自动 /aris-1-5-novelty-check 查新
```

用法：
```
> /aris-3-1-auto-review-loop 我的 diffusion model 论文
```
...
**涉及 Skills：** `aris-4-1-paper-plan` + `aris-4-2-paper-figure` + `aris-4-4-paper-write` + `aris-4-5-paper-compile` + `aris-4-6-auto-paper-improvement-loop` +（投稿后）`aris-5-3-paper-poster` + `aris-5-2-paper-slides`

> **一键调用：** `/aris-4-7-paper-writing "NARRATIVE_REPORT.md"` 自动跑完整个工作流 3。
...
```
NARRATIVE_REPORT.md ──► /aris-4-1-paper-plan ──► /aris-4-2-paper-figure ──► /aris-4-4-paper-write ──► /aris-4-5-paper-compile
...
2. /aris-4-1-paper-plan — 生成 claims-evidence 矩阵 + 分节计划
3. /aris-4-2-paper-figure — 生成对比表、训练曲线等图表
4. /aris-4-4-paper-write — 逐 section 生成 LaTeX（含 bib 清理、de-AI 打磨）
5. /aris-4-5-paper-compile — 编译 PDF、修复错误、页数验证
6. /aris-4-6-auto-paper-improvement-loop — 内容审稿 ×2 + 格式合规检查
```
...
> ⚠️ **`/aris-4-2-paper-figure` 能做什么、不能做什么：** 能自动生成**数据驱动的图表**（训练曲线、柱状图、热力图）和 **LaTeX 对比表**（从 JSON/CSV 数据）。**不能**生成架构图、流程图、模型示意图、生成样本网格——这些需要手动创建（draw.io、Figma、TikZ 等），放到 `figures/` 目录后再跑 `/aris-4-4-paper-write`。一篇典型 ML 论文中，约 60% 的图表可自动生成，约 40% 需手动制作。

**端到端实测：** 从一份 NARRATIVE_REPORT.md 生成了一篇 9 页 ICLR 2026 理论论文（7 节、29 条引用、4 张图、2 个对比表）——零编译错误、零 undefined reference。

#### 论文自动润色循环 ✨

工作流 3 生成论文后，`/aris-4-6-auto-paper-improvement-loop` 自动跑 2 轮 GPT-5.4 xhigh 内容审稿 → 修复 → 重编译，外加一轮格式合规检查，将粗稿自动提升到可投稿质量。
...
| Skill | 功能 | Codex MCP？ |
|-------|------|:---:|
| 🏗️ [`aris-0-1-research-pipeline`](skills/aris-0-1-research-pipeline/SKILL.md) | **端到端**：工作流 1 → 1.5 → 2 → 3，从研究方向到投稿 | 是 |

### 1.x Idea 阶段：发现与方案精炼 🔍

| Skill | 功能 | Codex MCP？ |
|-------|------|:---:|
| 🔭 **[`aris-0-2-idea-discovery`](skills/aris-0-2-idea-discovery/SKILL.md)** | **流水线编排** — 按顺序调用以下全部 skill | 是 |
| ├ 📚 [`aris-1-1-research-lit`](skills/aris-1-1-research-lit/SKILL.md) | 多源文献搜索（[Zotero](#-zotero-集成可选) + [Obsidian](#-obsidian-集成可选) + 本地 PDF + [arXiv API](#arxiv-集成) + 网络） | 否 |
| ├ 💡 [`aris-1-4-idea-creator`](skills/aris-1-4-idea-creator/SKILL.md) | 头脑风暴 8-12 个 idea，按可行性筛选，GPU pilot，按信号排序 | 是 |
| ├ 🔍 [`aris-1-5-novelty-check`](skills/aris-1-5-novelty-check/SKILL.md) | 多源查新 + GPT-5.4 交叉验证 | 是 |
| ├ 🔬 [`aris-1-6-research-review`](skills/aris-1-6-research-review/SKILL.md) | 单轮深度评审（外部 LLM，xhigh 推理） | 是 |
| └ 🧭 **[`aris-1-9-research-refine-pipeline`](skills/aris-1-9-research-refine-pipeline/SKILL.md)** | 方法精炼 + 实验规划一条龙 | 是 |
| 　├ 🔬 [`aris-1-7-research-refine`](skills/aris-1-7-research-refine/SKILL.md) | 冻结问题锚点 → 迭代精炼方法（最多 5 轮，≥9 分停） | 是 |
| 　└ 🧪 [`aris-1-8-experiment-plan`](skills/aris-1-8-experiment-plan/SKILL.md) | Claim-driven 实验路线图，含 ablation、预算和执行顺序 | 否 |

### 2.x 实验阶段：桥接与执行 🔗

| Skill | 功能 | Codex MCP？ |
|-------|------|:---:|
| 🔗 **[`aris-0-3-experiment-bridge`](skills/aris-0-3-experiment-bridge/SKILL.md)** | 读取实验计划 → 实现代码 → sanity check → 部署到 GPU → 收集初始结果 | 否 |
| ├ 🚀 [`aris-2-1-run-experiment`](skills/aris-2-1-run-experiment/SKILL.md) | 部署实验到本地（MPS/CUDA）或远程 GPU 服务器 | 否 |
| └ 👀 [`aris-2-2-monitor-experiment`](skills/aris-2-2-monitor-experiment/SKILL.md) | 监控实验进度、收集结果 | 否 |

### 3.x Review 阶段：自动科研循环 🔁

| Skill | 功能 | Codex MCP？ |
|-------|------|:---:|
| 🔁 **[`aris-3-1-auto-review-loop`](skills/aris-3-1-auto-review-loop/SKILL.md)** | **流水线编排** — 自动 review→修复→再 review（最多 4 轮） | 是 |
| ├ 🔬 [`aris-1-6-research-review`](skills/aris-1-6-research-review/SKILL.md) | 深度评审（与工作流 1 共用） | 是 |
| ├ 🔍 [`aris-1-5-novelty-check`](skills/aris-1-5-novelty-check/SKILL.md) | 审稿人建议新方向时验证新颖性 | 是 |
| ├ 🚀 [`aris-2-1-run-experiment`](skills/aris-2-1-run-experiment/SKILL.md) | 部署实验到本地（MPS/CUDA）或远程 GPU 服务器 | 否 |
| ├ 📊 [`aris-3-3-analyze-results`](skills/aris-3-3-analyze-results/SKILL.md) | 分析实验结果、统计、生成对比表 | 否 |
| └ 👀 [`aris-2-2-monitor-experiment`](skills/aris-2-2-monitor-experiment/SKILL.md) | 监控实验进度、收集结果 | 否 |
| 🔁 [`aris-3-4-auto-review-loop-llm`](skills/aris-3-4-auto-review-loop-llm/SKILL.md) | 同上，但使用任意 OpenAI 兼容 API，通过 [`llm-chat`](mcp-servers/llm-chat/) MCP 服务器 | 否 |

### 4.x 写作阶段：论文流水线 📝

| Skill | 功能 | Codex MCP？ |
|-------|------|:---:|
| 📝 **[`aris-4-7-paper-writing`](skills/aris-4-7-paper-writing/SKILL.md)** | **流水线编排** — 按顺序调用以下全部 skill | 是 |
| ├ 📐 [`aris-4-1-paper-plan`](skills/aris-4-1-paper-plan/SKILL.md) | Claims-evidence 矩阵、章节结构、图表计划、引用规划 | 是 |
| ├ 📊 [`aris-4-2-paper-figure`](skills/aris-4-2-paper-figure/SKILL.md) | 出版级 matplotlib/seaborn 图表 + LaTeX 对比表 | 可选 |
| ├ 🎨 [`aris-4-3-paper-illustration`](skills/aris-4-3-paper-illustration/SKILL.md) | AI 生成架构图/方法示意图（Gemini），`illustration: true` 时启用 | 否（需 Gemini API） |
| ├ ✍️ [`aris-4-4-paper-write`](skills/aris-4-4-paper-write/SKILL.md) | 逐 section LaTeX 生成（ICLR/NeurIPS/ICML）。通过 DBLP/CrossRef 反幻觉 BibTeX | 是 |
| ├ 🔨 [`aris-4-5-paper-compile`](skills/aris-4-5-paper-compile/SKILL.md) | 编译 LaTeX 为 PDF，自动修复错误，投稿就绪检查 | 否 |
| └ 🔄 [`aris-4-6-auto-paper-improvement-loop`](skills/aris-4-6-auto-paper-improvement-loop/SKILL.md) | 2 轮内容审稿 + 格式检查（4/10 → 8.5/10） | 是 |

### 5.x 投稿后阶段：Rebuttal 与回复 📝

| Skill | 功能 | Codex MCP？ |
|-------|------|:---:|
| 📝 **[`aris-5-1-rebuttal`](skills/aris-5-1-rebuttal/SKILL.md)** | 解析 review → 原子化 → 策略 → 起草 → 安全检查 → 压力测试 → 定稿（双版本）→ follow-up | 是 |

### 🛠️ 独立 / 工具类

| Skill | 功能 | Codex MCP？ |
|-------|------|:---:|
| 📄 [`aris-1-2-arxiv`](skills/aris-1-2-arxiv/SKILL.md) | 搜索、下载、摘要 arXiv 论文。可独立使用或作为 `/aris-1-1-research-lit` 补充 | 否 |
| 🎨 [`aris-9-3-pixel-art`](skills/aris-9-3-pixel-art/SKILL.md) | 生成像素风 SVG 插图，用于 README、文档或幻灯片 | 否 |
| 📱 [`aris-9-1-feishu-notify`](skills/aris-9-1-feishu-notify/SKILL.md) | [飞书](#-飞书lark-集成可选)推送（webhook）或双向交互。默认关闭 | 否 |
...
如果你用 [Zotero](https://www.zotero.org/) 管理论文，`/aris-1-1-research-lit` 可以搜索你的文献库、读取标注/高亮、导出 BibTeX——全在联网搜索之前完成。
...
**启用后 `/aris-1-1-research-lit` 新增能力：**
- 🔍 按主题搜索 Zotero 库（含语义/向量搜索）
- 📂 浏览 Collections 和 Tags
- 📝 读取你的 PDF 标注和高亮（你个人认为重要的内容）
- 📄 导出 BibTeX 供论文写作直接使用

**不用 Zotero？** 没关系——`/aris-1-1-research-lit` 自动跳过，用本地 PDF + 网络搜索。
...
如果你用 [Obsidian](https://obsidian.md/) 做研究笔记，`/aris-1-1-research-lit` 可以搜索你的 vault 中的论文总结、带标签的引用和你自己的洞察。
...
**不用 Obsidian？** 没关系——`/aris-1-1-research-lit` 自动跳过，照常工作。

> 💡 **Zotero + Obsidian 同时使用**：很多研究者用 Zotero 存论文、Obsidian 记笔记。两个集成可以同时工作——`/aris-1-1-research-lit` 先查 Zotero（原始论文 + 标注），再查 Obsidian（加工后笔记），再查本地 PDF，最后搜网络。

#### arXiv 集成

`/aris-1-1-research-lit` 自动通过 arXiv API 获取结构化元数据（标题、摘要、完整作者列表、分类），比网页搜索片段更丰富。无需额外配置。

默认只获取元数据（不下载文件）。如需同时下载最相关的 PDF：

```
/aris-1-1-research-lit "topic" — arxiv download: true                    # 下载 top 5 篇 PDF
/aris-1-1-research-lit "topic" — arxiv download: true, max download: 10  # 下载至多 10 篇
```

也可使用独立的 [`/aris-1-2-arxiv`](skills/aris-1-2-arxiv/SKILL.md) skill 直接搜索和下载：

```
/aris-1-2-arxiv "attention mechanism"           # 搜索
/aris-1-2-arxiv "2301.07041" — download         # 下载指定论文
```
...
| Skill | 事件 | 推送模式 | 交互模式 |
|-------|------|----------|----------|
| `/aris-3-1-auto-review-loop` | 每轮出分、循环结束 | 分数 + 结论 | + 等你决定继续/停止 |
| `/aris-4-6-auto-paper-improvement-loop` | 每轮出分、全部完成 | 分数进展表 | 分数进展表 |
| `/aris-2-1-run-experiment` | 实验已部署 | GPU 分配 + 预计时间 | GPU 分配 + 预计时间 |
| `/aris-2-2-monitor-experiment` | 结果已收集 | 结果对比表 | 结果对比表 |
| `/aris-0-2-idea-discovery` | 阶段切换、最终报告 | 各阶段摘要 | + 审批/拒绝 |
| `/aris-0-1-research-pipeline` | 阶段切换、流水线结束 | 阶段摘要 | + 审批/拒绝 |
...
> 💡 **参数自动透传**：参数沿调用链自动向下传递。例如 `/aris-0-1-research-pipeline "方向" — sources: zotero, arxiv download: true` 会将 `sources` 和 `arxiv download` 经 `aris-0-2-idea-discovery` 一路传到 `aris-1-1-research-lit`。你可以在任何层级设置下游参数——只需加 `— key: value`。
>
> ```
...
| 常量 | 默认值 | 说明 | 透传 |
|------|--------|------|:---:|
| `AUTO_PROCEED` | true | 用户不回复时自动带着最优方案继续 | → `aris-0-2-idea-discovery` |
| `ARXIV_DOWNLOAD` | false | 搜索后自动下载最相关的 arXiv PDF | → `aris-0-2-idea-discovery` → `aris-1-1-research-lit` |
| `HUMAN_CHECKPOINT` | false | 设为 `true` 时每轮 review 后暂停等待确认 | → `aris-3-1-auto-review-loop` |
| `WANDB` | false | 自动给实验脚本加 W&B 日志 | → `aris-0-3-experiment-bridge` → `aris-2-1-run-experiment` |
| `CODE_REVIEW` | true | GPT-5.4 部署前审查实验代码 | → `aris-0-3-experiment-bridge` |
| `BASE_REPO` | false | GitHub 仓库 URL，克隆作为实验基础代码 | → `aris-0-3-experiment-bridge` |
| `COMPACT` | false | 生成精简摘要文件，适合短 context 模型和 session 恢复 | → 所有工作流 |
| `REF_PAPER` | false | 参考论文（PDF 或 URL），先总结再基于它找 idea | → `aris-0-2-idea-discovery` |
| `ILLUSTRATION` | `gemini` | AI 作图：`gemini`（默认，需 API key）、`mermaid`（免费）、`false`（跳过） | → `aris-4-7-paper-writing` |

行内覆盖：`/aris-0-1-research-pipeline "方向" — auto proceed: false, wandb: true, illustration: true`

### 自动 Review 循环（`aris-3-1-auto-review-loop`）
...
### 找 Idea（`aris-0-2-idea-discovery` / `aris-1-4-idea-creator`）
...
| `ARXIV_DOWNLOAD` | false | 搜索后自动下载最相关的 arXiv PDF | → `aris-1-1-research-lit` |

行内覆盖：`/aris-0-2-idea-discovery "方向" — pilot budget: 4h per idea, sources: zotero, arxiv download: true`

### 实验桥接（`aris-0-3-experiment-bridge`）
...
行内覆盖：`/aris-0-3-experiment-bridge — code review: false, wandb: true`

### 文献搜索（`aris-1-1-research-lit`）
...
行内覆盖：`/aris-1-1-research-lit "方向" — sources: zotero, web`、`/aris-1-1-research-lit "方向" — arxiv download: true, max download: 10`

### 论文写作（`aris-4-4-paper-write`）
...
行内覆盖：`/aris-4-4-paper-write — target venue: NeurIPS, illustration: true`
...
> /aris-1-4-idea-creator, /aris-1-6-research-review, /aris-3-1-auto-review-loop, /aris-1-5-novelty-check,
> /aris-0-2-idea-discovery, /aris-0-1-research-pipeline, /aris-1-1-research-lit, /aris-2-1-run-experiment,
> /aris-3-3-analyze-results, /aris-2-2-monitor-experiment, /aris-9-3-pixel-art
> ```
...
- [ ] **常驻模式（Daemon mode）** — 通过 `launchd`/`systemd` 自动重启 Claude Code 会话，实现真正的无人值守运行（[#11](https://github.com/cd4374/aris-lite/issues/11)）
- [ ] **参考风格图表生成** — 参考论文 PDF 图表 → 识别配色/布局 → 生成同风格数据图表。方法示意图 ✅ 已由 `aris-4-3-paper-illustration` 解决
- [ ] **工作流执行报告** — 每个工作流完成后自动生成结构化总结：做了什么、关键决策、实验结果、评分和耗时。输出 `WORKFLOW_REPORT.md`，方便汇报
- [ ] **文档输入全流程** — 支持传入详细文档（如 `RESEARCH_BRIEF.md`）作为 `/aris-0-1-research-pipeline` 或 `/aris-0-2-idea-discovery` 的输入，替代一句话 prompt。很多研究方向需要详细的上下文（前期结果、约束条件、领域知识），一句话说不清楚。文档会被解析提取问题定义、约束、已有结果和具体需求
...
@misc{yang2026aris,
    ...
    year         = {2026},
    organization = {GitHub},
    url          = {https://github.com/cd4374/aris-lite},
}
```

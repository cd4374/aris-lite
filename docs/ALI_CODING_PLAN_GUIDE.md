...
| 过夜全自动循环（`/aris-3-1-auto-review-loop` 无人值守） | **违反** | 高，可能封禁 Key |
...
```
/aris-3-4-auto-review-loop-llm "你的论文主题"    # 已原生支持 llm-chat MCP
```
...
```bash
claude
> 读一下这个项目，验证 /aris-3-4-auto-review-loop-llm skill 是否正常可用，列出可识别的所有 skill
```

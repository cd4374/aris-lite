...
refine-logs/EXPERIMENT_TRACKER.md     code        /aris-2-1-run-experiment        for /aris-3-1-auto-review-loop
...
```
/aris-2-1-run-experiment [sanity experiment command]
```
...
```
/aris-2-1-run-experiment [experiment commands]
```

For each milestone:
1. Deploy experiments in parallel (up to MAX_PARALLEL_RUNS)
2. Use `/aris-2-2-monitor-experiment` to track progress
...
2. **Training quality check** — if W&B data is available, invoke `/aris-2-3-training-check` to detect NaN, loss divergence, plateaus, or overfitting. If W&B is not configured, skip silently.
...
- Ready for /aris-3-1-auto-review-loop: [YES/NO]
...
After main experiments (M2) complete with positive results, invoke `/aris-2-4-ablation-planner` to design ablation studies:
...
If `/aris-2-4-ablation-planner` is unavailable, skip silently.

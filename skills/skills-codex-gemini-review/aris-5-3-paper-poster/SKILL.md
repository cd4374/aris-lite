...
This skill runs **after** Workflow 3 (`/aris-4-7-paper-writing`). It takes a compiled paper and generates a print-ready poster for conference poster sessions. The poster extracts key content from the paper — it does **not** dump the full paper text onto a poster.
...
- **VENUE = `NeurIPS`** — Target venue, determines color scheme. Supported: `NeurIPS`, `ICML`, `ICLR`, `AAAI`, `ACL`, `EMNLP`, `CVPR`, `ECCV`, `GENERIC`. Override via argument (e.g., `/aris-5-3-paper-poster "— venue: ICML"`).
...
> 💡 Override: `/aris-5-3-paper-poster "paper/" — venue: CVPR, size: A1, orientation: portrait, columns: 3`
...
> This phase uses **Claude visual assessment** on rendered poster images to iteratively refine layout, readability, and visual hierarchy — similar to the `aris-4-3-paper-illustration` skill's review loop.
...
Send the poster content plan + key LaTeX sections to Gemini review for review.
...
```
/aris-5-3-paper-poster "paper/" — venue: CVPR, size: A1, orientation: portrait, columns: 3
```

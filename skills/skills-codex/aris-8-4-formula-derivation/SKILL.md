---
name: aris-8-4-formula-derivation
...
Do **not** use this skill as a replacement for strict proof writing once the exact claim is already fixed and the user wants a theorem-proof package. In that case, hand off to `aris-8-5-proof-writer`.
...
## Relationship to `aris-8-5-proof-writer`

Use `aris-8-4-formula-derivation` when the user says things like:
...
Use `aris-8-5-proof-writer` only after:

...
This file is the source of truth for `/aris-2-1-run-experiment` and `/aris-2-2-monitor-experiment` to connect to vast.ai instances.

## Workflow

### Action: Provision (default)

Analyze the task, find the best GPU, and present cost-optimized options. This is the main entry point — called directly or automatically by `/aris-2-1-run-experiment` when `gpu: vast` is set.

**Step 1: Analyze Task Requirements**

Read available context to determine what the task needs:

1. **From the experiment plan** (`02_EXPERIMENT_PLAN.md`, fallback: `refine-logs/EXPERIMENT_PLAN.md`):
...
To deploy: /aris-2-1-run-experiment (will auto-detect this instance)
To destroy when done: /aris-2-5-vast-gpu destroy <ID>
```

### Action: Setup

Set up the rented instance for a specific experiment. Called automatically by `/aris-2-1-run-experiment` when targeting a vast.ai instance.
...
```
/aris-2-1-run-experiment "train model"       ← detects gpu: vast, calls /aris-2-5-vast-gpu provision
  ↳ /aris-2-5-vast-gpu provision             ← analyzes task, presents options with cost
  ↳ user picks option               ← rent + setup + deploy
  ↳ /aris-2-5-vast-gpu destroy               ← auto-destroy when done (if auto_destroy: true)

/aris-2-5-vast-gpu provision                 ← manual: analyze task + show options
/aris-2-5-vast-gpu rent <offer_id>           ← manual: rent a specific offer
/aris-2-5-vast-gpu list                      ← show active instances
/aris-2-5-vast-gpu destroy <instance_id>     ← tear down, stop billing
/aris-2-5-vast-gpu destroy-all               ← tear down everything
```

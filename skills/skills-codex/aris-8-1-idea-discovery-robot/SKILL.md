---
name: aris-8-1-idea-discovery-robot
...
```
/aris-1-1-research-lit → /aris-1-4-idea-creator (robotics framing) → /aris-1-5-novelty-check → /aris-1-6-research-review
...
> Override inline, e.g. `/aris-8-1-idea-discovery-robot "bimanual manipulation" — only sim ideas, no real robot` or `/aris-8-1-idea-discovery-robot "drone navigation" — focus on CoRL/RSS, 2 pilot ideas max`
...
```
/aris-1-1-research-lit "$ARGUMENTS — focus venues: CoRL, RSS, ICRA, IROS, RA-L, TRO, Science Robotics"
```
...
```
/aris-1-4-idea-creator "$ARGUMENTS — robotics frame: [paste Robotics Problem Frame] — focus venues: CoRL, RSS, ICRA, IROS, RA-L — benchmark-specific ideas only — sim-first pilots — no real-robot execution without explicit approval — require failure metrics and baseline clarity"
```
...
```
/aris-1-5-novelty-check "[idea description with embodiment + task family + benchmark + sensor stack + controller/policy class + sim2real angle + target venues: CoRL/RSS/ICRA/IROS/RA-L]"
```
...
```
/aris-1-6-research-review "[top idea with robotics framing, embodiment, benchmark, baselines, pilot plan, evaluation metrics, and sim2real/hardware risks — review as CoRL/RSS/ICRA reviewer]"
```
...
- [ ] Run /aris-1-5-novelty-check on the final idea wording
...
```
/aris-8-1-idea-discovery-robot "direction"   ← you are here
implement sim-first pilot
/aris-2-1-run-experiment                     ← if infrastructure exists
/aris-3-1-auto-review-loop "top robotics idea"
```

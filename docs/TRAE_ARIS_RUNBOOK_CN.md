...
**Trae 等价写法：**
```
使用 aris-0-2-idea-discovery 技能，运行完整的 idea discovery 流程，方向："your research direction"。

按顺序使用以下子技能：
1. 使用 aris-1-1-research-lit 技能 —— 文献综述
2. 使用 aris-1-4-idea-creator 技能 —— 头脑风暴
3. 使用 aris-1-5-novelty-check 技能 —— 新颖性验证
4. 使用 aris-1-6-research-review 技能 —— 深度评审
5. 使用 aris-1-9-research-refine-pipeline 技能 —— 方法精化 + 实验规划
```

> **提示：** 如果上下文过长，可以将每个阶段拆分为单独的对话，通过文件（如 `01_IDEA_REPORT.md`、`01_FINAL_PROPOSAL.md`）传递结果。
...
**Trae 等价写法：**
```
使用 aris-0-3-experiment-bridge 技能。
读取 02_EXPERIMENT_PLAN.md（回退：`refine-logs/EXPERIMENT_PLAN.md`）并实现实验。
使用 aris-2-1-run-experiment 技能部署到 GPU。
```
...
**Trae 等价写法：**
```
使用 aris-3-1-auto-review-loop 技能。
对 "your paper topic" 运行自动评审循环。
读取项目叙事文档、记忆文件和实验结果。
使用 MCP 工具 mcp__codex__codex 进行外部审阅。
```

> **注意：** 如果使用 `llm-chat` MCP，把 `mcp__codex__codex` 替换为 `mcp__llm-chat__chat`。或使用适配版技能：`aris-3-4-auto-review-loop-llm`。
...
**Trae 等价写法：**
```
使用 aris-4-7-paper-writing 技能。
输入：项目根目录的 04_NARRATIVE_REPORT.md（回退：`NARRATIVE_REPORT.md`）。

按顺序使用以下子技能：
1. 使用 aris-4-1-paper-plan 技能 —— 大纲 + claims-evidence matrix
2. 使用 aris-4-2-paper-figure 技能 —— 生成图表
3. 使用 aris-4-4-paper-write 技能 —— 写 LaTeX 章节
4. 使用 aris-4-5-paper-compile 技能 —— 编译 PDF
5. 使用 aris-4-6-auto-paper-improvement-loop 技能 —— 审阅与润色
```
...
| 阶段 | 执行方式 | 产出文件 |
|------|---------|---------|
| 1 | 创意发现：使用 `aris-0-2-idea-discovery` 技能 + 研究方向 | `01_IDEA_REPORT.md`, `01_FINAL_PROPOSAL.md`, `02_EXPERIMENT_PLAN.md` |
| 2 | 实验桥接：使用 `aris-0-3-experiment-bridge` 技能 | `02_EXPERIMENT_RESULTS.md`、更新后的 `02_EXPERIMENT_TRACKER.md` |
| 3 | 自动评审：使用 `aris-3-1-auto-review-loop` 技能 | `03_AUTO_REVIEW.md`, `04_NARRATIVE_REPORT.md` |
| 4 | 论文写作：使用 `aris-4-7-paper-writing` 技能 + `04_NARRATIVE_REPORT.md` | `paper/` 目录，`06_SUBMISSION_GATE.md` |
...
| `03_AUTO_REVIEW.md` | 累计审阅日志 | auto-review-loop |
| `01_IDEA_REPORT.md` | 创意筛选与初评结果 | idea-discovery |
| `05_PAPER_PLAN.md` | 论文大纲与 claim-evidence matrix | paper-plan |
| `paper/PAPER_IMPROVEMENT_LOG.md` | 论文改进回合日志 | auto-paper-improvement-loop |
...
```text
@skills/aris-2-1-run-experiment/SKILL.md
...
```
# 文献综述
使用 aris-1-1-research-lit 技能，搜索 "discrete diffusion models" 相关论文。

# 创意发现（完整流程）
使用 aris-0-2-idea-discovery 技能，对 "factorized gap in discrete diffusion LMs" 运行创意发现。

# 单次深度评审
使用 aris-1-6-research-review 技能，评审我的研究：[描述或指向文件]。

# 自动评审循环
使用 aris-3-1-auto-review-loop 技能，运行自动评审循环。课题："your paper topic"。

# 论文写作
使用 aris-4-7-paper-writing 技能，根据 04_NARRATIVE_REPORT.md 写论文。

# 部署实验
使用 aris-2-1-run-experiment 技能，部署：python train.py --lr 1e-4 --epochs 100
```
...
- [ ] 快速测试：`使用 aris-1-6-research-review 技能评审我的项目`
